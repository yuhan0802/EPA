...
def attention_visualization(atten, img):
    atten = np.mean(atten, axis=2)
    atten = np.float32(atten)
    atten_map = cv2.cvtColor(atten, cv2.COLOR_RGB2BGR)
    normed_mask = atten_map / np.max(atten_map)
    normed_mask = np.uint8(255 * normed_mask)
    normed_mask = cv2.applyColorMap(normed_mask, cv2.COLORMAP_JET)
    normed_mask = cv2.addWeighted(img, 0.6, normed_mask, 1.0, 0)
    return cv2.cvtColor(normed_mask, cv2.COLOR_BGR2RGB)


def image_to_video(img_folder, fps, name):
    img_names = os.listdir(img_folder)
    img_names.sort()
    h, w, _ = cv2.imread(os.path.join(img_folder, img_names[0])).shape
    img_array = [cv2.imread(os.path.join(img_folder, i)) for i in img_names]
    video = cv2.VideoWriter(('/data/' + name + '.avi'), cv2.VideoWriter_fourcc(*'MJPG'), fps,
                            (w, h))  # 定义保存视频目录名称及压缩格式，fps=10,像素为1280*720
    for i in img_array:
        video.write(i)  # 写入视频
    video.release()
    print('Finish!')
